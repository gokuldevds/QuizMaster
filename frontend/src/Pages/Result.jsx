...
const Result = () => {
  ...
  const { quizTitle, totalScore, correct, incorrect, total, answers } = location.state || {};
    ...
    <div className="min-h-screen bg-gray-50 text-gray-800">
      ...
      <header className="flex items-center justify-between px-10 py-6 border-b bg-white shadow-sm">
        ...
        <nav className="flex items-center space-x-8 text-gray-600 font-medium">
          ...
          <a href="/quizzes" className="hover:text-indigo-700">Quizzes</a>
          <a href="/leaderboard" className="hover:text-indigo-700">Leaderboard</a>
        </nav>
      ...
      <main className="max-w-5xl mx-auto p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
          <div className="bg-white p-8 rounded-xl shadow-sm h-48 flex items-center justify-center text-gray-400">
              ...
              <tbody>
                {answers.map((item, index) => (
                  <tr
                    key={index}
                    className={`border-t ${
                      item.isCorrect ? "bg-white" : "bg-red-50"
                    }`}
                  >
